...
RC RelationManager::deleteTable(const string &tableName)
{
    ...
    // Delete all existing indexes for this table
    vector<IndexFile> indexFiles;
    rc = getIndexFiles(tableName, indexFiles);
    if (rc)
        return rc;

    FileHandle fh;
    rbfm->openFile (getFileName(INDEXES_TABLE_NAME), fh);

    for (IndexFile indexFile : indexFiles)
    {
        destroyIndex(tableName, indexFile.attrName);
    }
...
RC RelationManager::insertTuple(const string &tableName, const void *data, RID &rid)
{
    ...
    if (rc)
    {
        cerr << "Failed to get attributes" << endl;
        return rc;
    }
    ...
    if (rc)
    {
        cerr << "Failed to open rbfm file" << endl;
        return rc;
    }
    ...
    // insert into all indexes as well
    map<string, AttributeData> fields;
    vector<AttributeData> attrData;
    rbfm->getAttributeData(recordDescriptor, data, attrData);
    ...
    // Get existing indexes, loop over each index and insert field[name].data if not null
    vector<IndexFile> indexFiles;
    rc = getIndexFiles(tableName, indexFiles);
    if (rc)
    {
        cerr << "failed to get index files" << endl;
        return rc;
    }

    for (IndexFile indexFile : indexFiles)
    {
        AttributeData d = fields[indexFile.attrName];
        if (d.null)
            continue;
        IXFileHandle ifh;
        rc = im->openFile(indexFile.fileName, ifh);
        if (rc)
        {
            cerr << "Failed to open index file" << endl;
            return rc;
        }
        rc = im->insertEntry(ifh, d.attr, d.data, rid);
        if (rc)
        {
            cerr << "Failed to insert into index" << endl;
            return rc;
        }
        rc = im->closeFile(ifh);
        if (rc)
        {
            cerr << "Failed to close index file" << endl;
            return rc;
        }
    }
...
RC RelationManager::deleteTuple(const string &tableName, const RID &rid)
{
    ...
    // Get all indexes, read each attribute with an associated index and delete the key/rid pair
    // from each index
    ...
    // Get existing indexes, loop over each index and delete field[name].data if not null
    vector<IndexFile> indexFiles;
    rc = getIndexFiles(tableName, indexFiles);
    if (rc)
        return rc;
    ...
    for (IndexFile indexFile : indexFiles)
    {
        ...
        if (rc)
            return rc;
        rc = im->deleteEntry(ifh, d.attr, d.data, rid);
        if (rc)
            return rc;
        rc = im->closeFile(ifh);
        if (rc)
    ...
    // delete from all indexes as well
...
RC RelationManager::updateTuple(const string &tableName, const void *data, const RID &rid)
{
    ...
    rbfm->readRecord(fileHandle, recordDescriptor, rid, oldData);

    map<string, AttributeData> oldFields;
    vector<AttributeData> attrDataOld;
    rbfm->getAttributeData(recordDescriptor, oldData, attrDataOld);

    for (AttributeData ad : attrDataOld)
    {
        oldFields[ad.attr.name] = ad;
    }

    // Get new data
    map<string, AttributeData> newFields;
    vector<AttributeData> attrDataNew;
    rbfm->getAttributeData(recordDescriptor, data, attrDataNew);

    for (AttributeData ad : attrDataNew)
    {
        newFields[ad.attr.name] = ad;
    }
    ...
    for (IndexFile indexFile : indexFiles)
    {
        AttributeData old = oldFields[indexFile.attrName];
        AttributeData newer = newFields[indexFile.attrName];
        
        IXFileHandle ifh;
        rc = im->openFile(indexFile.fileName, ifh);
        if (rc)
            return rc;
        // Delete old data if it wasn't null
        if (!old.null)
            rc = im->deleteEntry(ifh, old.attr, old.data, rid);
        if (rc)
            return rc;
        // Insert new data if it isn't null
        if (!newer.null)
            rc = im->insertEntry(ifh, newer.attr, newer.data, rid);
        if (rc)
            return rc;
        rc = im->closeFile(ifh);
        if (rc)
...
{
    return tableName + "_" + attrName + string(INDEX_FILE_EXTENSION);
}
...
{
    string filename = getIndexFileName(tablename, attrname);

    unsigned offset = 0;
    int32_t tnameLen = tablename.length(), anameLen = attrname.length(), fnameLen = filename.length();
    ...
    memcpy((char*) data + offset, &tnameLen, VARCHAR_LENGTH_SIZE);
    offset += VARCHAR_LENGTH_SIZE;
    memcpy((char*) data + offset, tablename.c_str(), tnameLen);
    offset += tnameLen;

    memcpy((char*) data + offset, &anameLen, VARCHAR_LENGTH_SIZE);
    offset += VARCHAR_LENGTH_SIZE;
    memcpy((char*) data + offset, attrname.c_str(), anameLen);
    offset += anameLen;

    memcpy((char*) data + offset, &fnameLen, VARCHAR_LENGTH_SIZE);
    offset += VARCHAR_LENGTH_SIZE;
    memcpy((char*) data + offset, filename.c_str(), fnameLen);
    offset += fnameLen;
}
...
// Inserts an index into the Index catalog
RC RelationManager::insertIndex(const string &tablename, const string &attrname)
{
    ...
    RID rid;
    prepareIndexRecordData(tablename, attrname, indexData);
...
void RelationManager::toAPI(const string &str, void *data)
{
    ...
    memcpy((char*) data, &len, INT_SIZE);
    memcpy((char*) data + INT_SIZE, str.c_str(), len);
}

void RelationManager::toAPI(const int32_t integer, void *data)
{
    memcpy((char*) data, &integer, INT_SIZE);
}

void RelationManager::toAPI(const float real, void *data)
{
    memcpy((char*) data, &real, REAL_SIZE);
}
...
RC RelationManager::getAttribute(const string &tableName, const string &attributeName, Attribute &attr)
{
    ...
    return RM_NO_SUCH_ATTR;
}

// Gets all existing index files for the given table
RC RelationManager::getIndexFiles(const string &tablename, vector<IndexFile> &indexFiles)
{
    indexFiles.clear();

    vector<string> projection {INDEXES_COL_ATTR_NAME, INDEXES_COL_FILE_NAME};

    void *value = malloc(tablename.length() + 4);
    toAPI(tablename, value);

    // Scan through index catalog
    RM_ScanIterator iter;
    RC rc = scan(INDEXES_TABLE_NAME, INDEXES_COL_TABLE_NAME, EQ_OP, value, projection, iter);
    if (rc)
    {
        cerr << "Failed to scan" << endl;
        free (value);
        return rc;
    }
    ...
    while ((rc = iter.getNextTuple(rid, data)) == (SUCCESS))
    {
        char null = 0;
        memcpy (&null, data, 1);
        if (null)
        ...
        char attrName[len+1];
        attrName[len] = '\0';
        memcpy(attrName, (char*)data + offset, len);
        offset += len;
        ...
        char fname[len+1];
        fname[len] = '\0';
        memcpy(fname, (char*)data + offset, len);

        // Package them up with an RID
        IndexFile indexFile;
        indexFile.rid = rid;
        indexFile.fileName = fname;
        indexFile.attrName = attrName;
                              ...
                              RM_IndexScanIterator &rm_IndexScanIterator)
{
    ...
    RC rc = im->openFile(fileName, rm_IndexScanIterator.fh);
    if (rc)
        return rc;

    Attribute attr;
    rc = getAttribute(tableName, attributeName, attr);
    if (rc)
        return rc;

    rc = im->scan(rm_IndexScanIterator.fh, attr, lowKey, highKey, lowKeyInclusive, highKeyInclusive, rm_IndexScanIterator.ix_iter);
...
RC RelationManager::createIndex(const string &tableName, const string &attributeName)
{
    ...
    IXFileHandle fh;
    rc = im->openFile(fileName, fh);
    if (rc)
        return rc;

    // Iterate through the rbfm file and grab all of the existing records
    vector<string> projection = {attributeName};
    RM_ScanIterator iter;
    scan(tableName, "", NO_OP, NULL, projection, iter);

    RID rid;
    void *data = malloc(attr.length + 5);

    // Insert these key,rid pairs into the newly created index
    while ((rc = iter.getNextTuple(rid, data)) == SUCCESS)
    {
        // If this attribute is null, skip it
        if (*(char*)data)
            continue;
        rc = im->insertEntry(fh, attr, (char*)data + 1, rid);
        if (rc)
            break;
    }
    if (rc == IX_EOF)
...
RC RelationManager::destroyIndex(const string &tableName, const string &attributeName)
{
    ...
    string filename = getIndexFileName(tableName, attributeName);
    // Destroy index file
    RC rc = im->destroyFile(filename);
    if (rc)
        return rc;

    void *value = malloc(filename.length() + 4);
    toAPI(filename, value);

    // Find entry in index catalog and delete it from catalog
    vector<string> projection = {INDEXES_COL_FILE_NAME};
    RM_ScanIterator iter;
    scan(INDEXES_TABLE_NAME, INDEXES_COL_FILE_NAME, EQ_OP, value, projection, iter);

    RID rid;
    void *data = malloc(PAGE_SIZE);
    if ((rc = iter.getNextTuple(rid, data)) == SUCCESS)
    {
        FileHandle fh;
        rbfm->openFile(getFileName(INDEXES_TABLE_NAME), fh);
