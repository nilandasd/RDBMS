...
// Added for project 4
void RecordBasedFileManager::getAttributeData(const vector<Attribute> &recordDescriptor, const void *data, vector<AttributeData> &attrData)
{
    attrData.clear();
    ...
    {
        AttributeData ad;
        ad.attr = recordDescriptor[i];
        ad.null = fieldIsNull(nullIndicator, i);
        if (ad.null)
        {
            ad.data = NULL;
            attrData.push_back(ad);
            continue;
        }

        ad.data = (char*)data + offset;
